import { CircleCheckBig, Film } from "lucide-react";
...
const SingleMovie = () => {
  ...
  const handleAddToWatchlist = async (id: string) => {
    try {
      ...
      await API.post("/watchlists", { movieId: id });
        ...
        <>
          <div className="w-full h-max text-center relative">
            <div className="absolute top-0 left-0 -z-10 w-full h-full">
              <img src={data.movie.image} alt="movie image" className="w-full h-full" />
          ...
          <div className="movie-display h-max">
            <div
              className="flex flex-col justify-center items-center"
              data-aos="fade-up-left"
              data-aos-duration="1000"
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-[90%] justify-start gap-3">
                <MovieInfo name="Director" info={data.movie.director} />
                <MovieInfo name="Ratings" info={`${data.movie.rating} / 10`} />
                <MovieInfo
                  name="Genres"
                ...
                <MovieInfo name="Added here by" info={data.movie.Owner.name} />
              </div>
            </div>
            {/* <div
